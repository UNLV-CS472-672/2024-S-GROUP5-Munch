...
def get_our_own_businesses(latitude, longitude):
    ...
    response = requests.get(
        f"{URL}businesses/search?latitude={latitude}&longitude={longitude}&limit={limit}&offset={random_int}&term=restaurants",
        headers=headers,
    )
    ...
    if response.status_code == 200:
        ...
        for r in response.json()["businesses"]:
            ...
            if "categories" in r:
                r["description"] = ", ".join(
                    category["title"] for category in r["categories"]
                )

            # filter response to only keep the fields we want
            filtered_response = {
                key: value for key, value in r.items() if key in kept_fields
            }
