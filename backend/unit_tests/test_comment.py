...
class CommentTest(TestCase):
  ...
  def test_comment(self):
    ...
    create_user = self.client.put(f"/api/users/{testing_user_id}", json=user_data)
    ...
    # Comment data
    new_comment_data = {
      "comment" : "This is a new comment for comment test"
    }

    # Append endpoint to post id and test patch comment
    post_id = post_id[len("posts/"):]
    patch_comment = self.client.patch(f"/api/posts/comment/{testing_user_id}/{post_id}", json=new_comment_data)
    ...
    patch_comment = self.client.delete(f"/api/posts/comment/{testing_user_id}/{post_id}/{comment_id}", json=new_comment_data)
    self.assertEqual(patch_comment.status_code, status.HTTP_200_OK)
    
    # Testing Errors
    error_post = self.client.patch(f"api/posts/comment/{testing_user_id}/ThisDoesntWork")
    self.assertEqual(error_post.status_code, status.HTTP_404_NOT_FOUND)
    
    error_post = self.client.patch(f"api/posts/comment/ThisDoesntWork/{post_id}")
    self.assertEqual(error_post.status_code, status.HTTP_404_NOT_FOUND)
    
    error_post = self.client.delete(f"api/posts/comment/{testing_user_id}/ThisDoesntWork/{comment_id}")
    self.assertEqual(error_post.status_code, status.HTTP_404_NOT_FOUND)
    
    error_post = self.client.delete(f"api/posts/comment/ThisDoesntWork/{post_id}/{comment_id}")
    self.assertEqual(error_post.status_code, status.HTTP_404_NOT_FOUND)