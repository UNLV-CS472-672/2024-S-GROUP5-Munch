...
class TestLikeRoutes(unittest.TestCase):
    ...
    def test_like_and_unlike_post(self):
        ...
        create_post_response = self.client.post("/api/posts", json=new_post_mock_data)
        ...
        # Like the post
        like_response = self.client.patch(f"/api/users/{testing_user_id}/like/{post_id}")
        self.assertEqual(like_response.status_code, status.HTTP_200_OK)
        self.assertEqual(like_response.json["message"], "Post liked successfully")

        # Unlike the post
        unlike_response = self.client.patch(f"/api/users/{testing_user_id}/unlike/{post_id}")
        self.assertEqual(unlike_response.status_code, status.HTTP_200_OK)
        self.assertEqual(unlike_response.json["message"], "Post unliked successfully")
        ...
        error_like = self.client.patch(f"/api/users/{testing_user_id}/like/invalid_post")
        self.assertEqual(error_like.status_code, status.HTTP_404_NOT_FOUND)
        
        error_like = self.client.patch(f"/api/users/{testing_user_id}/unlike/invalid_post")
        self.assertEqual(error_like.status_code, status.HTTP_404_NOT_FOUND)
        ...
        error_like = self.client.patch(f"/api/users/invalid_user/like/{post_id}")
        self.assertEqual(error_like.status_code, status.HTTP_404_NOT_FOUND)
        
        error_like = self.client.patch(f"/api/users/invalid_user/unlike/{post_id}")
        self.assertEqual(error_like.status_code, status.HTTP_404_NOT_FOUND)
