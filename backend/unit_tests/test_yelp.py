...
class YelpTest(TestCase):
    ...
    def test_yelp(self):
        ...
        get_local_businesses = self.client.get(f"/api/{valid_latitude}/{valid_longitude}")
        self.assertEqual(get_local_businesses.status_code, status.HTTP_200_OK)
        
        # Error check local businesses
        get_local_businesses = self.client.get(f"/api/InvalidLatitude/InvalidLongitude")
        self.assertEqual(get_local_businesses.status_code, status.HTTP_400_BAD_REQUEST)