...
export default function CommentComponent({
  ...
  return (
    <View width={width} height={"auto"} flexDirection='row' alignItems="center" flex={1} marginLeft="$3" marginBottom="$3">
      <View marginLeft={"$4"} flex={1}>
        <Text marginBottom={'$1.5'}>
          <Text fontWeight={'bold'} fontSize={14}>{name}</Text> 
          <Text color={'$gray10'} fontSize={12}>  {getDateDifference(creation_date)}</Text>
        </Text>
        <Text>{text}</Text>
      </View>
      {children}
